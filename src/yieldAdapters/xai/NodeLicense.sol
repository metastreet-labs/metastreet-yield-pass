...
contract NodeLicense is ERC721EnumerableUpgradeable, AccessControlUpgradeable {
    ...
    // Whitelist of addresses that are allowed to transfer from or to
    EnumerableSetUpgradeable.AddressSet private _whitelist;

    // Referee
    IReferee private referee;
    ...
    function mint(uint256 _amount, string calldata _promoCode) public payable {
        require(_tokenIds.current() + _amount <= maxSupply, "Exceeds maxSupply");
    ...
    function price(uint256 _amount, string calldata _promoCode) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < pricingTiers.length; i++) {
            ...
            uint256 availableInThisTier = tierSum > totalSupply ? tierSum - totalSupply : 0;
    ...
    /**
     * @notice Override the transfer function to only allow addresses that are in the white list in the to or from field
     * to go through if they are not staked
    ...
    function _transfer(address from, address to, uint256 tokenId) internal override {
        ...
        require(referee.assignedKeyToPool(tokenId) == address(0), "Transfer not allowed: key needs to be unstaked");
